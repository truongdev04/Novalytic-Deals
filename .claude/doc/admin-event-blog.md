...
## 6. Session update — 2026-07-11 / 2026-07-12

**Event — tách `Event.couponId String[]` thành bảng join `EventCoupon`**
- Lý do: `couponId` là mảng denormalized, không phải FK thật — xoá 1 store phải tự viết code dò từng event có chứa coupon id của store đó rồi lọc mảng thủ công (`lib/data/stores.ts` `deleteStore`); xoá 1 coupon lẻ (không qua xoá store) thì hoàn toàn không dọn, để lại id rác trong `Event.couponId`.
- `prisma/schema.prisma`: thêm model `EventCoupon` (`eventId`, `couponId`, `createdAt`, `updatedAt`, PK kép `(eventId, couponId)`, cả 2 FK `onDelete: Cascade`, `@@map("event_coupons")`), bỏ hẳn cột `Event.couponId`. `Event`/`Coupon` thêm relation `eventCoupons EventCoupon[]`.
- Migration `20260711160842_split_event_coupons_join_table`: tạo bảng `event_coupons`, backfill từ `events.couponId` cũ bằng `unnest`, xoá trước các dòng backfill trỏ tới coupon id không còn tồn tại (rác cũ do gap nêu trên) rồi mới thêm FK, cuối cùng drop cột `couponId` trên `events`. Đã `prisma migrate deploy` thành công lên Supabase thật (backfill 38 dòng).
- Sự cố phát sinh lúc deploy (không liên quan việc này): 2 migration `deals` trước đó (`20260711034826_add_deal`, `20260711113227_deal_category_offer`) đã thực sự chạy xong trên DB (bảng `deals` đủ cột) nhưng bảng `_prisma_migrations` chưa đánh dấu `finished_at` (drift cũ) → `migrate deploy` báo lỗi P3018 chặn đường. Khắc phục bằng `prisma migrate resolve --applied <name>` cho cả 2 (không chạy lại SQL, chỉ đánh dấu) rồi mới deploy migration mới — không có thay đổi schema nào khác ngoài dự kiến.
- `lib/data/events.ts`: `toEvent` nhận thêm tham số `couponIds` (trước đây đọc thẳng `row.couponId`); `getEvents`/`getEventBySlug`/`getEventById`/`updateEvent` query thêm bảng `EventCoupon` (batch qua `couponIdsByEventId()` khi list nhiều event, tránh N+1 — cùng pattern với `storeIdsByEventId()` có sẵn). `unionCouponsIntoEvent()` đổi từ merge mảng trong JS sang `prisma.eventCoupon.createMany({skipDuplicates:true})`. `setEventCoupons()` (replace-all) đổi sang transaction `deleteMany` + `createMany`.
- `lib/data/stores.ts` `deleteStore()`: **bỏ hẳn** đoạn code thủ công tìm event chứa coupon id của store rồi lọc mảng — giờ FK cascade (`Coupon` xoá → cascade `EventCoupon`) tự lo, hàm chỉ còn query slug của các event bị ảnh hưởng để purge đúng cache tag.
- `lib/data/coupons.ts` `deleteCoupon()`: không cần sửa gì — tự động được vá lỗ hổng cũ (xoá coupon lẻ giờ cascade dọn `EventCoupon` luôn, trước đây phải chờ xoá cả store mới dọn).
- `prisma/seed.ts` `seedEvents()`: tạo `EventCoupon` qua nested `create: { eventCoupons: { create: [...] } }` thay vì gán thẳng mảng `couponId`.
- Không cần sửa `EventForm.tsx`, API routes admin (`app/api/admin/events/*`), hay trang public `/events/[slug]` — tầng UI/API chỉ thấy `event.featuredCouponIds: string[]` (type `Event` ở `types/event.ts` không đổi), toàn bộ chuyển đổi Prisma gói gọn trong `lib/data/events.ts`.
- Verify: `npm run typecheck` và `npm run lint` sạch (chỉ còn đúng 1 warning cũ không liên quan như mọi session trước); sau khi đổi schema đã xoá `.next/dev/cache/fetch-cache` + restart dev server (bắt buộc, đúng quy trình đã ghi nhận từ session 2026-07-07); curl `/`, `/events`, `/api/blog` đều 200.
- **Chưa** verify tay trên UI thật (Featured Coupons picker, xoá store có event, xoá coupon lẻ) — chỉ mới qua typecheck/lint/curl + kiểm tra trực tiếp số dòng bảng `event_coupons` sau backfill.

**Admin Sidebar — padding-bottom**
- `components/admin/AdminSidebar.tsx` L134: `<nav>` thêm `pb-24` — mục cuối cùng ("Footer" trong submenu Settings) trước đó bị nút nổi (floating widget) ở góc dưới trái che mất khi cuộn hết sidebar.

**Bước tiếp theo (session 2026-07-11/12)**
f. User cần tự tay xác nhận trên UI thật: xoá 1 store có gán event + có exclusive coupon → coupon biến mất khỏi Featured Coupons của event đúng như trước (không còn coupon rác); Featured Coupons picker trong `EventForm` vẫn thêm/bớt đúng; sidebar cuộn xuống cuối thấy đủ khoảng đệm, không bị nút nổi che "Footer".
g. Các mục (b) và (c) ở Bước tiếp theo của session 2026-07-06 vẫn còn nguyên, chưa làm.
